...
async function publishFeedStep(
  ...
  // Routine cap/cooldown rejections aren't bugs — downgrade so the public feed
  // doesn't fill with red "Order failed" rows that look like the system is broken.
    ...
    if (pipelineResult.decision && pipelineResult.action === "analyzed") {
      ...
      if (decision.action === "buy" && decision.marketId) {
        ...
        const buyResult = await executeBuy(
          decision, ctx.agentId, ctx.jobId, ctx.agentWalletId, ctx.ownerPubkey, portfolio, AGENT_NAME, "sports"
        );

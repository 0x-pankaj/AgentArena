...
export async function runGeneralAgentTick(
  ...
  if (fsm.getState() === "ANALYZING") {
    ...
    const thresholdCheck = checkThresholds(signals, lastAnalysisTime, markets, positions, "general", consecutiveNoEdge);
    ...
    await publishFeedStep(ctx.agentId, "thinking", `${AGENT_NAME} 🔍 Stage 1/2: Starting research + analysis phase — analyzing ${markets.length} markets...`, { pipeline_stage: "research_analysis_start" });

    const researchAnalysisPrompt = buildGeneralResearchContext(signals, markets, positions, portfolio.totalBalance, thresholdCheck.reasons);
    ...
    // Aggregate signals — Features 1+2+9: Calibrated weights + signal decay + confidence calibration
    ...
    const signalAgeMinutes = signals.fetchedAt
      ...
      : 0;
    const calibratedWeights = await getAllCalibratedWeights("general");

    const defaultGeneralWeights: Record<string, number> = {
      llm_analysis: 3.0, gdelt_sentiment: 1.0, conflict_signal: 1.5,
    };
    const decayedSignals = [
      { name: "llm_analysis", value: extractProbabilityFromText(researchAnalysis.text), confidence: decayConfidence(0.7, signalAgeMinutes, "market"), weight: calibratedWeights["llm_analysis"] ?? defaultGeneralWeights["llm_analysis"] ?? 3.0 },
      { name: "gdelt_sentiment", value: gdeltToProbability(signals.gdelt), confidence: decayConfidence(0.5, signalAgeMinutes, "gdelt"), weight: calibratedWeights["gdelt_sentiment"] ?? defaultGeneralWeights["gdelt_sentiment"] ?? 1.0 },
      { name: "conflict_signal", value: conflictToProbability(signals.acled), confidence: decayConfidence(0.6, signalAgeMinutes, "acled"), weight: calibratedWeights["conflict_signal"] ?? defaultGeneralWeights["conflict_signal"] ?? 1.5 },
    ...
    const temporalAdj = computeGeneralTemporalAdjustment(markets);

    await publishFeedStep(ctx.agentId, "signal_update", `${AGENT_NAME} ✅ Signal aggregation complete (calibrated + decayed + temporal adjusted)`, { 
    ...
    // STAGE 2/2: Trade decision (structured output)
    ...
    try {
      ...
      const result = await quickDecision<TradeDecision>({
        ...
        userMessage: buildGeneralDecisionContext(researchAnalysis.text, finalAggregatedSignal, markets, positions, portfolio.totalBalance, temporalAdj),
      ...
      const llmConfidenceAdj = await getConfidenceAdjustment("general", config.models.decision.model, decision.confidence);
    ...
    // FAST-PATH: Skip adversarial + consensus if strong edge and high confidence
    ...
    if (useFastPath) {
      await publishFeedStep(ctx.agentId, "thinking", `${AGENT_NAME} ⚡ FAST-PATH: edge=${(preEdge.netEdge * 100).toFixed(1)}% > ${(FAST_PATH_EDGE_THRESHOLD * 100).toFixed(0)}% AND confidence=${(decision.confidence * 100).toFixed(0)}% >= ${(FAST_PATH_CONFIDENCE_THRESHOLD * 100).toFixed(0)}% — skipping adversarial + consensus`, { 
        pipeline_stage: "fast_path_triggered", 
    ...
    if (decision.marketId) {
      const scenarioMarketPrice = decision.isYes ? getMarketPrice(markets, decision.marketId, "yes") : getMarketPrice(markets, decision.marketId, "no");
      ...
      const scenarioResult = runScenarioAnalysis({
        ...
        marketPrice: scenarioMarketPrice,
    ...
    if (decision.marketId) {
      await recordAgentPrediction("general", ctx.agentId, decision, signals, markets, config.models.decision.model).catch((err) =>
        console.error(`[General Agent] Failed to record prediction: ${err.message}`)
      );
    }
    ...
    if (decision.action === "buy") {
      ...
      if (result.success) {
        if (result.positionId) {
          ...
          }).catch((err) => console.error(`[General Agent] Failed to register position monitoring: ${err.message}`));
        }

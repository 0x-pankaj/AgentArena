...
import { getCachedJupiterEvents, invalidateCategoryCache } from "./jupiter-cache-manager";
...
export async function getTrendingMarkets(params: {
  ...
  try {
    const jupiterCategory = category ?? 'general';
    const { events } = await getCachedJupiterEvents(jupiterCategory, { maxEvents: limit });

    const markets = eventsToMarkets(events, category);
    await redis.setex(cacheKey, CACHE_TTL_SECONDS.trending, JSON.stringify(markets));
    return { markets };
  } catch {
    // Fall back to DB
    const rows = await db
